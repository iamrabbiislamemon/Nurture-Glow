...
- **Vaccine Progress**: Percentage of vaccines with status "Taken"
...
Track vaccination schedules for mother and baby.
...
#### Key Features
- **Vaccine Catalog**: Loaded from `app_catalog` (type: `vaccines-list`) in MySQL
- **Smart Suggestions**: Based on current pregnancy week from `vaccine_suggestions` table
- **Status Toggle**: One-click switch between Taken/Pending/Missed
- **Due Date Filtering**: Vaccines due within 14 days highlighted
- **Search & Sort**: Filter by status, sort by date ascending/descending
...
| `app_catalog` | Product catalog, vaccine catalog, etc. |
