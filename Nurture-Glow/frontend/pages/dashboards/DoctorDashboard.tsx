...
import VaccineApprovals from '../../components/dashboards/doctor/VaccineApprovals';
import DoctorProfileSettings from '../../components/dashboards/doctor/DoctorProfileSettings';
...
const DoctorDashboard: React.FC = () => {
  ...
  // Upcoming consultations sub-tab
  const [upcomingSubTab, setUpcomingSubTab] = useState<'pending' | 'accepted' | 'rejected' | 'missed'>('pending');
  ...
  const {
    ...
    missedConsultations: missedConsultationsRaw,
    recentPatients,
    earnings
  } = dashboardData;

  const missedConsultations = missedConsultationsRaw || [];

  const displayName = profile.name || profile.email || '';
  const cleanName = displayName.replace(/^Dr\.?\s*/i, '').trim();
  const headlineName = cleanName ? `Dr. ${cleanName}` : 'Doctor';
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-hero-content">
            ...
            <div className="bg-gradient-to-b from-white/80 via-white/70 to-white/60 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200/40 p-6">
              ...
              {/* Sub-tabs */}
              {(() => {
                const pendingList = upcomingConsultations.filter(c => c.status === 'pending');
                const acceptedList = upcomingConsultations.filter(c => c.status === 'scheduled' || c.status === 'in-progress');
                const rejectedList = upcomingConsultations.filter(c => c.status === 'cancelled');
                const missedList = missedConsultations;

                const subTabs = [
                  { key: 'pending' as const, label: 'Pending', count: pendingList.length, color: 'amber' },
                  { key: 'accepted' as const, label: 'Accepted', count: acceptedList.length, color: 'emerald' },
                  { key: 'rejected' as const, label: 'Rejected', count: rejectedList.length, color: 'red' },
                  { key: 'missed' as const, label: 'Missed', count: missedList.length, color: 'slate' },
                ];

                const activeList =
                  upcomingSubTab === 'pending' ? pendingList :
                  upcomingSubTab === 'accepted' ? acceptedList :
                  upcomingSubTab === 'rejected' ? rejectedList :
                  missedList;

                return (
                  <>
                    <div className="flex items-center gap-2 mb-5 bg-gray-100/80 p-1 rounded-xl">
                      {subTabs.map(tab => (
                        <button
                          key={tab.key}
                          onClick={() => setUpcomingSubTab(tab.key)}
                          className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all ${
                            upcomingSubTab === tab.key
                              ? tab.color === 'amber'
                        ...
                        >
                          ...
                          {tab.key === 'accepted' && <CheckCircle size={14} />}
                          {tab.key === 'rejected' && <XCircle size={14} />}
                          {tab.key === 'missed' && <AlertCircle size={14} />}
                          {tab.label}
                    ...
                    {activeList.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-500">
                          {upcomingSubTab === 'pending' && 'No pending requests'}
                          {upcomingSubTab === 'accepted' && 'No accepted consultations'}
                          {upcomingSubTab === 'rejected' && 'No rejected requests'}
                          {upcomingSubTab === 'missed' && 'No missed consultations'}
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {activeList.map((consultation) => {
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
                                isPending
                                  ? 'bg-amber-50/50 border-amber-200/50 hover:shadow-md'
                                  : isScheduled
                                  ? 'bg-emerald-50/50 border-emerald-200/50 hover:shadow-md'
                                  : 'bg-red-50/30 border-red-200/40'
                              }`}
                            >
                              <div className="flex items-center gap-3">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                                  isPending ? 'bg-amber-100' : isScheduled ? 'bg-emerald-100' : 'bg-red-100'
                                }`}>
                                  <Users size={20} className={
                                    isPending ? 'text-amber-600' : isScheduled ? 'text-emerald-600' : 'text-red-400'
                                  } />
                                </div>
                                <div>
                              ...
                              <div className="flex items-center gap-3">
                                {isPending && (
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={() => handleConsultationStatusChange(consultation.id, 'scheduled')}
                                      className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl text-xs font-bold transition-all shadow-sm"
                                    >
                                      Accept
                                    </button>
                                    <button
                                      onClick={() => handleConsultationStatusChange(consultation.id, 'cancelled')}
                                      className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl text-xs font-bold transition-all shadow-sm"
                                    >
                                      Reject
                                    </button>
                                  </div>
                                ...
                                {isScheduled && (
                                  <span className="px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold uppercase">Approved</span>
                                )}
                                {isCancelled && (
                                  <span className="px-3 py-1.5 bg-red-100 text-red-600 rounded-lg text-xs font-bold uppercase">Rejected</span>
                                )}
                                <div className="text-right">
                                  <p className={`text-sm font-medium ${isCancelled ? 'text-gray-400' : 'text-gray-900'}`}>
