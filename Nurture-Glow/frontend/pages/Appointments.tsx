...
const Appointments: React.FC = () => {
  ...
  // My Appointments sub-tab
  const [myApptSubTab, setMyApptSubTab] = useState<'pending' | 'approved' | 'rejected' | 'missed'>('approved');
  ...
  const normalizeAppointmentStatus = (status: Appointment['status'] | string) => {
    ...
    if (normalized.includes('scheduled') || normalized.includes('approved')) return 'Approved';
    if (normalized.includes('upcoming') || normalized.includes('in-progress') || normalized.includes('in progress')) {
      return 'Upcoming';
    }
    return 'Upcoming';
  };

  // Check if an appointment's date/time has passed (used to derive "Missed" display status)
  ...
  // Get the display status — derives "Missed" from Approved/Upcoming appointments whose date has passed
  const getDisplayStatus = (app: Appointment) => {
    const status = normalizeAppointmentStatus(app.status);
    if ((status === 'Approved' || status === 'Upcoming') && isAppointmentPast(app)) {
      return 'Missed';
    }
    return status;
  };
  const isReviewEligible = (app: Appointment) => {
    ...
    if (displayStatus === 'Cancelled' || displayStatus === 'Pending') return false;
    if (displayStatus === 'Completed') return true;
    // Missed appointments can also be rated
    if (displayStatus === 'Missed') return true;
    return false;
  };
  ...
  const bookedAppointments = dailyAppointments.filter(
    (a) => { const s = normalizeAppointmentStatus(a.status); return s === 'Approved' || s === 'Upcoming'; }
  );
  const pendingAppointments = dailyAppointments.filter(
    (a) => normalizeAppointmentStatus(a.status) === 'Pending'
  );
  const rejectedAppointments = dailyAppointments.filter(
    (a) => normalizeAppointmentStatus(a.status) === 'Cancelled'
  );

  // All appointments across ALL dates, split into 3 groups
  const allPendingAppointments = useMemo(() => {
    return appointments
      .filter((a) => normalizeAppointmentStatus(a.status) === 'Pending')
      .sort((a, b) => {
        const da = a.date || '';
        const db = b.date || '';
  ...
  const allApprovedAppointments = useMemo(() => {
    return appointments
      .filter((a) => {
        const ds = getDisplayStatus(a);
        return ds === 'Approved' || ds === 'Upcoming' || ds === 'Completed';
      })
      .sort((a, b) => {
        const da = a.date || '';
        const dB = b.date || '';
  ...
  const allRejectedAppointments = useMemo(() => {
    return appointments
      .filter((a) => getDisplayStatus(a) === 'Cancelled')
      .sort((a, b) => {
        const da = a.date || '';
        const dB = b.date || '';
  ...
  const allMissedAppointments = useMemo(() => {
    return appointments
      .filter((a) => getDisplayStatus(a) === 'Missed')
      .sort((a, b) => {
        const da = a.date || '';
        const dB = b.date || '';
        if (da !== dB) return dB.localeCompare(da); // newest first
  ...
  // Set of date strings that have appointments (for calendar dots)
          ...
          <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 space-y-6">
            ...
            <div className="grid grid-cols-7 gap-1 text-center">
              ...
              {calendarDays.map((date, i) => {
                ...
                const hasAppt = date ? appointmentDatesSet.has(date.toISOString().split('T')[0]) : false;
                return (
                  <div key={i} className="aspect-square flex items-center justify-center">
                    {date ? (
                      <button
                        ...
                        disabled={isPast}
                        className={`w-9 h-9 rounded-xl text-xs font-bold transition-all cursor-pointer relative ${
                      ...
                      >
                        ...
                        {hasAppt && (
                          <span className={`absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${
                            isSameDay(date, selectedFilterDate) ? 'bg-white' : 'bg-teal-500'
                          }`} />
                        )}
                      </button>
          ...
          {activeTab !== 'my' ? (
            filteredDoctors.length === 0 ? (
              <div className="bg-white p-16 rounded-[40px] text-center border-2 border-dashed border-gray-100 space-y-4 animate-in fade-in col-span-full">
                ...
                <p className="text-gray-400 font-medium text-lg">No doctors available for {activeTab} consultations yet.</p>
                <p className="text-sm text-gray-400">Doctors need to set up their profile and schedule first.</p>
              </div>
            ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredDoctors.map(doc => (
                <div key={doc.id} className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 hover:border-[#BFE6DA] transition-all group">
                  ...
                  {doc.bio && (
                    <p className="text-xs text-gray-500 mb-4 line-clamp-2">{doc.bio}</p>
                  )}
            ...
            <div className="space-y-5">
              ...
              {(() => {
                const subTabs = [
                  { key: 'pending' as const, label: 'Pending', count: allPendingAppointments.length, color: 'amber' },
                  { key: 'approved' as const, label: 'Approved', count: allApprovedAppointments.length, color: 'teal' },
                  { key: 'rejected' as const, label: 'Rejected', count: allRejectedAppointments.length, color: 'red' },
                  { key: 'missed' as const, label: 'Missed', count: allMissedAppointments.length, color: 'slate' },
                ];

                const activeList =
                  myApptSubTab === 'pending' ? allPendingAppointments :
                  myApptSubTab === 'approved' ? allApprovedAppointments :
                  myApptSubTab === 'rejected' ? allRejectedAppointments :
                  ...
                  <>
                    <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100">
                      {subTabs.map(tab => (
                        <button
                          key={tab.key}
                          onClick={() => setMyApptSubTab(tab.key)}
                    ...
                    {activeList.length === 0 ? (
                      <div className="bg-white p-16 rounded-[40px] text-center border-2 border-dashed border-gray-100 space-y-4 animate-in fade-in">
                        ...
                        <p className="text-gray-400 font-medium text-lg italic">
                          {myApptSubTab === 'pending' && 'No pending appointment requests.'}
                          {myApptSubTab === 'approved' && 'No approved appointments yet.'}
                          {myApptSubTab === 'rejected' && 'No rejected appointments.'}
                          {myApptSubTab === 'missed' && 'No missed appointments.'}
                        </p>
                        {myApptSubTab !== 'rejected' && (
                          <p className="text-sm text-gray-400">Book a doctor from the Online or Offline consult tabs.</p>
                        )}
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {activeList.map((app) => {
                          ...
                          const isApproved = displayStatus === 'Approved' || displayStatus === 'Upcoming';
                          const isCompleted = displayStatus === 'Completed';
                          const isCancelled = displayStatus === 'Cancelled';
                          const isMissed = displayStatus === 'Missed';

                          const statusBadge = isPending ? 'Pending approval'
                            : isApproved ? 'Approved'
                            ...
                            <div key={app.id} className={`bg-white p-5 rounded-[28px] shadow-sm border ${cardBorder} flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-all animate-in slide-in-from-bottom-2`}>
                              <div className="flex items-center gap-4 w-full md:w-auto">
                                ...
                                <div className="min-w-0">
                                  <h4 className={`font-bold text-sm ${isCancelled ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{app.doctorName}</h4>
                                  <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">{app.specialty} • {app.type}</p>
                                  <div className="flex items-center gap-2 mt-1.5">
                                    <span className={`inline-flex px-3 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${statusClass}`}>
                                      {statusBadge}
                                    </span>
                                  </div>
                                </div>
                              ...
                              <div className="flex flex-wrap items-center gap-3 md:gap-6 w-full md:w-auto">
                                <div className="flex items-center gap-4">
                                  <div className="flex flex-col items-center bg-gray-50 px-3 py-1.5 rounded-xl">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase">
                                      {app.date ? new Date(app.date + 'T00:00:00').toLocaleDateString('default', { month: 'short' }) : ''}
                                    </span>
                                    <span className="text-base font-extrabold text-gray-800 leading-none">
                                      {app.date ? new Date(app.date + 'T00:00:00').getDate() : ''}
                                    </span>
                                  </div>
                                ...
                                <div className="flex gap-2">
                                  {(isApproved || isPending) && !isMissed && app.type === 'Online' && (
                                    <VideoSessionButton
                                  ...
                                  {(isApproved || isPending) && !isMissed && (
                                    <button
                                      type="button"
                                      onClick={() => setCancelConfirmId(app.id)}
                                      className="px-5 py-2 bg-red-50 text-red-600 rounded-xl text-xs font-bold hover:bg-red-100 transition-all cursor-pointer"
                                    >
                                      Cancel
                                    </button>
                                  )}
                                  {isReviewEligible(app) && !reviewByAppointmentId.get(app.id) && (
                                    <button
                                      ...
                                      className="px-5 py-2 bg-amber-50 text-amber-700 rounded-xl text-xs font-bold hover:bg-amber-100 transition-all cursor-pointer"
                                    >
                                      Rate Doctor
                                    </button>
                                  )}
                                  {reviewByAppointmentId.get(app.id) && (
                                    <div className="px-3 py-2 bg-amber-50 text-amber-700 rounded-xl text-xs font-bold flex items-center gap-2">
                                      <Star size={14} className="text-amber-400 fill-amber-400" />
