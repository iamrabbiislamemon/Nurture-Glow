...
import { Search, Filter, Calendar, Video, Phone, Clock, FileText, ChevronDown, CheckCircle, XCircle, Play } from 'lucide-react';
...
const ConsultationList: React.FC = () => {
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      setUpdatingId(id);
      await DoctorDashboardService.updateConsultationStatus(id, newStatus as Consultation['status']);
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {(statusLabel === 'pending') && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleStatusUpdate(consultation.id, 'scheduled'); }}
                          disabled={updatingId === consultation.id}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-green-500 hover:bg-green-600 text-white text-xs font-bold transition-all disabled:opacity-50"
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleStatusUpdate(consultation.id, 'cancelled'); }}
                          disabled={updatingId === consultation.id}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-red-500 hover:bg-red-600 text-white text-xs font-bold transition-all disabled:opacity-50"
                    ...
                    {(statusLabel === 'scheduled') && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleStatusUpdate(consultation.id, 'in-progress'); }}
                          disabled={updatingId === consultation.id}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-teal-500 hover:bg-teal-600 text-white text-xs font-bold transition-all disabled:opacity-50"
                    ...
                    {(statusLabel === 'in-progress') && (
                      <button
                        onClick={(e) => { e.stopPropagation(); handleStatusUpdate(consultation.id, 'completed'); }}
                        disabled={updatingId === consultation.id}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold transition-all disabled:opacity-50"
