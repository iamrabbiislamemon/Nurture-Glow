...
# Configuration
$MYSQL_USER = "root"
$MYSQL_PASSWORD = ""  # Update if you have a password
...
if (Test-Path $adminSchemaPath) {
    if ($MYSQL_PASSWORD) {
        Get-Content $adminSchemaPath | mysql -u $MYSQL_USER -p$MYSQL_PASSWORD $DB_NAME 2>&1 | Out-Null
    } else {
...
if (Test-Path $testDataPath) {
    if ($MYSQL_PASSWORD) {
        Get-Content $testDataPath | mysql -u $MYSQL_USER -p$MYSQL_PASSWORD $DB_NAME 2>&1 | Out-Null
    } else {
...
Set-Location "d:\Nurture-Glow\Nurture-Glow\backend"
...
Set-Location "d:\Nurture-Glow\Nurture-Glow\Nurture-Glow"
...
# Step 5: Launch Admin Dashboards
Write-Host "STEP 5: Launching Admin Dashboards..." -ForegroundColor Yellow
Write-Host "─────────────────────────────────────────────────────────────────" -ForegroundColor Gray
...
Start-Process "msedge" -ArgumentList "--new-window", "$baseUrl/#/admin/login", "--profile-directory=`"Profile 1`""
Start-Sleep -Seconds 1

# Open Operations Admin
Write-Host "  → Operations Admin Dashboard" -ForegroundColor Magenta
Start-Process "msedge" -ArgumentList "--new-window", "$baseUrl/#/admin/login", "--profile-directory=`"Profile 2`""
Start-Sleep -Seconds 1

# Open Medical Admin
Write-Host "  → Medical Admin Dashboard" -ForegroundColor Magenta
Start-Process "msedge" -ArgumentList "--new-window", "$baseUrl/#/admin/login", "--profile-directory=`"Profile 3`""

Write-Host "`n✓ All admin dashboards launched!" -ForegroundColor Green
...
Write-Host "`n📋 Login Credentials (Use these to test each admin type):" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "STEP 7: Testing Workflow" -ForegroundColor Yellow
Write-Host "─────────────────────────────────────────────────────────────────" -ForegroundColor Gray

Write-Host "`n🔍 Recommended Testing Flow:" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "STEP 8: Useful Monitoring Commands" -ForegroundColor Yellow
Write-Host "─────────────────────────────────────────────────────────────────" -ForegroundColor Gray

Write-Host "`n📊 Database Queries (Run in MySQL):" -ForegroundColor Cyan
Write-Host ""
Write-Host "# Check admin users" -ForegroundColor Gray
Write-Host "SELECT id, email, role, status FROM users WHERE role LIKE '%admin%';" -ForegroundColor White
Write-Host ""
Write-Host "# Check admin actions log" -ForegroundColor Gray
Write-Host "SELECT * FROM admin_actions ORDER BY created_at DESC LIMIT 10;" -ForegroundColor White
Write-Host ""
Write-Host "# Check admin notifications" -ForegroundColor Gray
Write-Host "SELECT * FROM admin_notifications WHERE is_read = FALSE;" -ForegroundColor White
Write-Host ""
Write-Host "# Check security events" -ForegroundColor Gray
Write-Host "SELECT * FROM security_events WHERE resolved = FALSE;" -ForegroundColor White
Write-Host ""
...
Write-Host "`n⚠️  NOTE: If registration is required, use the invite code: NURTURE_ADMIN_2026" -ForegroundColor Yellow

Write-Host "`nPress any key to exit this setup script (servers will continue running)..." -ForegroundColor Gray
